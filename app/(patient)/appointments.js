...
export default function AppointmentsScreen() {
  ...
  const handleAdd = async () => {
    ...
    setModal(false); setTitle(''); setDoctor(''); setDate(''); setNotes('');
    load();
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.list}>
        {appointments.length === 0 && <Text style={styles.empty}>No appointments. Tap + to book one.</Text>}
        {appointments.map(a => (
          <View key={a.id} style={styles.card}>
            <View style={styles.dateBox}>
              <Text style={styles.dateNum}>{a.date?.split('-')[2] || '--'}</Text>
              <Text style={styles.dateMonth}>{MONTHS[(parseInt(a.date?.split('-')[1]) - 1)] || ''}</Text>
            </View>
      ...
      <Modal visible={modal} transparent animationType="slide">
        <View style={styles.overlay}>
          <View style={styles.modalBox}>
            ...
            {[
              { ph: 'Appointment title *', val: title, set: setTitle },
              { ph: 'Doctor name *', val: doctor, set: setDoctor },
              { ph: 'Date (YYYY-MM-DD) *', val: date, set: setDate },
              { ph: 'Notes (optional)', val: notes, set: setNotes },
            ].map((f, i) => (
              <TextInput key={i} style={styles.input} placeholder={f.ph} placeholderTextColor={COLORS.subtext} value={f.val} onChangeText={f.set} />
            ))}
            <View style={styles.row}>
              ...
              <TouchableOpacity style={styles.saveBtn} onPress={handleAdd}><Text style={styles.saveText}>Book</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  dateNum: { fontSize: FONTS.large, fontWeight: 'bold', color: COLORS.white },
  dateMonth: { fontSize: 12, color: 'rgba(255,255,255,0.8)' },
